    ...
    def get_some_vehicles_state(self, t: int, vids: list) -> list:
        ...
        for vid in vids:
            try:  # 如果找不到目标的话，说明该车已经到达的目的地，那么返回0
                ...
                res[4] = res[4] if type(
                    self.get_one_vehicle_state(t, vid)[2]) != -1 else self.config.road_length
                states.append(res)
            except KeyError:
                res = [0, 0, -2, 0, self.config.road_length, 0]
                states.append(res)
        return states
    ...
    def get_some_vehicle_position(self, t: int, vids: list):
        ...
        for vid in vids:
            try:
                ...
                res = [0, 0, -2, 0, self.config.road_length, 0, 0, 0]
...
if __name__ == '__main__':
    ...
    print(vehicle_state.get_one_vehicle_state(10, '12'))
