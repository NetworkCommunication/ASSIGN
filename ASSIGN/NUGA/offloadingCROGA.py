...
class Population(object):
    ...
    def crossover(self, p=0.9, num_offsprings=2):
        offsprings = []
        for i in range(0, self.p_size, 2):
            for _ in range(num_offsprings):
                offspring = Individual(self.adapter)
                for n in range(self.adapter.N):
                    if np.random.random() < p:
                        offspring.individual[n] = self.pop[i].individual[n].copy()
                    else:
                        offspring.individual[n] = self.pop[i + 1].individual[n].copy()
                offsprings.append(offspring)
        # 将交叉得到的子代添加到当前种群中
        self.pop.extend(offsprings)

    def mutate(self, p=0.9):
        for i in range(self.p_size):
            if np.random.random() < p:
                for n in range(self.adapter.N):
                    if np.random.random() < p:
